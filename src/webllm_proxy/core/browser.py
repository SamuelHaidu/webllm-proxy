...
and a single page can't be driven concurrently). Flask handlers submit jobs and
drain a per-job queue, so the SSE crosses threads. The transport is generic: it
opens the provider's URL, verifies auth, captures the provider's chosen response
over CDP, and feeds the bytes through the provider's accumulator — nothing here
...
_DUMP = os.environ.get("WEBLLM_PROXY_DUMP_SSE")
...
class BrowserSession:
    ...
    def _run(self):
        ...
        while True:
            ...
            if item[0] == "eval":
                js, arg, reply = item[1]
                try:
                    reply.put(self._page.evaluate(js, arg) if arg is not None
                              else self._page.evaluate(js))
    ...
    def _boot(self):
        ...
        profile.mkdir(parents=True, exist_ok=True)
        clean_singleton_locks(profile)
        log.info("[%s] launching CloakBrowser (headless=%s) profile=%s",
                 p.name, p.headless, profile)
        ...
        if self.provider.capture_match(resp.get("url", "")):
            ...
            for k, v in (resp.get("headers") or {}).items():
                if k.lower() == "content-type":
                    ct = v
                    break
            a["job"].out.put(("meta", {"status": resp.get("status", 200),
                                       "content_type": ct or "text/event-stream"}))
            try:
    ...
    def submit(self, payload) -> "queue.Queue":
        job = Job(payload)
    ...
    def _do_send(self, job: Job):
        self._active = {"job": job, "acc": self.provider.make_accumulator(),
                        "rid": None, "finished": False}
        a = self._active
        try:
        ...
        while not a["finished"]:
            ...
            if a["rid"] is None and now > idle_deadline:
                job.out.put(("error", "no response started")); job.out.put(None)
                a["finished"] = True
                break
        self._active = None

        ...
        json.dumps(
            {
                ...
                "target": "update",
                "arguments": [
                    {"messages": [{"author": "bot", "messageType": "Chat", "text": "Hello"}]}
                ],
            }
        )
        + DELIM
        ...
        json.dumps(
            {
                ...
                "target": "update",
                "arguments": [
                    {"messages": [{"author": "bot", "messageType": "Chat", "text": "Hello world"}]}
                ],
            }
        )
        + DELIM
                ...
                "item": {
                    "messages": [
                        {"author": "user", "text": "hi"},
                        {
                            ...
                            "text": "Hello world",
                            "sourceAttributions": [
                                {"providerDisplayName": "Example", "seeMoreUrl": "https://e.com"}
                            ],
...
def test_signalr_encode_send_shape():
    c = SignalRCodec()
    frame = c.encode_send(
        "hi", conversation_id="c1", options={"tone": "Magic", "source": "officeweb"}
    )[0]
...
def test_event_incremental_deltas_and_final():
    c = EventCodec()
    assert isinstance(
        c.decode(json.dumps({"event": "startMessage", "conversationId": "cX"}))[0], Ack
    )
...
def test_event_hashcash_challenge_roundtrip():
    c = EventCodec()
    got = c.decode(
        json.dumps({"event": "challenge", "method": "hashcash", "parameter": "abc:1", "id": "0.1"})
    )
...
def test_m365_model_discovery_parses_manifest():
    ...
    assert (
        models["Gpt_5_5_Reasoning"].reasoning is True
        and models["Gpt_5_5_Reasoning"].family == "GPT"
    )

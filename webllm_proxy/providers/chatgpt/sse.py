...
class V1DeltaParser:
    ...
    def _add_message_text(self, content):
        """The text of a freshly-`add`ed message, from whichever shape it
        arrived in (see the three cases commented below)."""
        parts = content.get("parts") or []
        if parts and isinstance(parts[0], str) and parts[0]:
            return parts[0]
        # Reasoning delivered as content.thoughts[] (each a {summary, content}
        # block) rather than parts -- the thinking model's chain-of-thought.
        # content_type is "thoughts" so _emit routes it to reasoning.
        if isinstance(content.get("thoughts"), list):
            return self._thoughts_text(content["thoughts"])
        # A native tool call (recipient != all) whose whole payload arrives in
        # one `add` as content.text with content_type "code" and NO parts (e.g.
        # `container.exec`: `bash -lc ...`). The parts-only read misses it, so
        # pull content.text here.
        if self.cur_recipient not in (None, "all") and isinstance(content.get("text"), str):
            return content["text"]
        return ""
    ...
    # `url` marker shapes: <START>url<SEP>label<SEP>href<END> (labeled link) or
    # <START>url<SEP>href<END> (bare link).
...
class StreamAccumulator(Accumulator):
